  ...
  return (
    <PageTransition>
      ...
      <section className="relative w-full h-[60vh] md:h-[70vh] pt-16 overflow-hidden bg-brand-black">
        ...
        <button onClick={prevImage} className="absolute left-5 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/20 backdrop-blur text-white flex items-center justify-center hover:bg-black/40 transition-colors z-10">
          <ChevronLeft size={24} />
        </button>
        <button onClick={nextImage} className="absolute right-5 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/20 backdrop-blur text-white flex items-center justify-center hover:bg-black/40 transition-colors z-10">
      ...
      <section className="bg-white py-10 px-5 md:px-10 xl:px-20 border-b border-brand-rule">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
          <motion.div variants={slideLeft} initial="hidden" animate="visible">
            <h1 className="text-3xl md:text-4xl font-bold text-brand-black mb-2">{villa.name}</h1>
            <p className="text-brand-mid text-lg">{villa.tagline}</p>
          </motion.div>
          <motion.div variants={slideRight} initial="hidden" animate="visible" className="flex flex-wrap gap-3">
            {[
              ...
              <div key={i} className="bg-brand-surface rounded-full px-4 py-1.5 text-sm font-medium text-brand-black">
                {spec}
              </div>
            ))}
          </motion.div>
      ...
      <section className="bg-brand-surface py-20 md:py-28 px-5 md:px-10 xl:px-20">
        <div className="max-w-7xl mx-auto space-y-24">
          {[
            ...
            <div key={i} className={`flex flex-col ${i % 2 !== 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-12 lg:gap-16 items-center`}>
              <motion.div 
              ...
              <motion.div 
                ...
                className="w-full lg:w-1/2"
              >
                <h3 className="text-3xl font-bold text-brand-black mb-4">{space.title}</h3>
                <p className="text-lg text-brand-mid leading-relaxed">{space.desc}</p>
              </motion.div>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="flex items-center gap-4 mb-4">
              ...
              />
              <span className="text-xs font-semibold uppercase tracking-widest text-brand-muted">Everything Included</span>
            </div>
            <h2 className="text-4xl xl:text-5xl font-bold leading-tight mb-4">No Detail Overlooked</h2>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {[
              { title: "Kitchen", items: ["Refrigerator", "Gas Stove", "Microwave", "Water Purifier", "Cutlery & Crockery", "Utensils"] },
              { title: "Entertainment", items: ["2 Smart TVs", "High-Speed WiFi", "Washing Machine"] },
              { title: "Climate", items: ["AC in All Rooms", "Ceiling Fans"] },
              { title: "Convenience", items: ["Power Backup (Inverter)", "CCTV Security", "Covered Parking", "Iron & Ironing Board", "Rocking Chair"] },
              { title: "Service", items: ["Daily Housekeeping", "Caretaker On-Site", "Private Chef (on request)", "Villa Manager Contact"] },
            ].map((category, i) => (
              <motion.div 
                key={i}
                variants={fadeUp}
                ...
                viewport={{ once: true }}
              >
                <h4 className="text-xs font-semibold uppercase tracking-widest text-brand-muted mb-6">{category.title}</h4>
                <ul className="space-y-4">
                  {category.items.map((item, j) => (
                    <li key={j} className="flex items-center gap-3 text-brand-black font-medium">
      ...
      <section className="bg-brand-surface py-20 md:py-28 px-5 md:px-10 xl:px-20">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="flex items-center justify-center gap-4 mb-4">
              <motion.div
                ...
                className="h-px w-12 bg-brand-rule origin-left"
              />
              <span className="text-xs font-semibold uppercase tracking-widest text-brand-muted">Every Balcony, A Different World</span>
              <motion.div
                initial={{ scaleX: 0 }}
                whileInView={{ scaleX: 1 }}
                viewport={{ once: true }}
            ...
            <h2 className="text-4xl xl:text-5xl font-bold leading-tight mb-6">The Views</h2>
            <p className="text-lg text-brand-mid leading-relaxed">
              Porvorim sits where the hills meet the sea. From the balconies of Familia Furtado, you'll catch all three.
            </p>
          </motion.div>
          ...
          >
            {[
              { img: "https://images.unsplash.com/photo-1576013551627-11dc368f0d47?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80", label: "Pool View" },
              { img: "https://images.unsplash.com/photo-1518182170546-076616fdca44?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80", label: "Valley View" },
              { img: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80", label: "Partial Sea View" },
            ].map((view, i) => (
              <motion.div key={i} variants={fadeUp} className="relative rounded-2xl overflow-hidden aspect-[4/5]">
                <img src={view.img} alt={view.label} className="absolute inset-0 w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                <div className="absolute bottom-6 left-6">
                  <h3 className="text-white font-semibold text-xl">{view.label}</h3>
                </div>
              </motion.div>
            ))}
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          <motion.div variants={slideLeft} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <div className="flex items-center gap-4 mb-4">
              <motion.div
                ...
                className="h-px w-12 bg-brand-rule origin-left"
              />
              <span className="text-xs font-semibold uppercase tracking-widest text-brand-muted">Booking</span>
            </div>
            ...
            <p className="text-lg text-brand-mid leading-relaxed mb-8">
              Check availability or simply get in touch. We respond personally — no automated replies, no waiting rooms.
            </p>
            <ul className="space-y-4 mb-8">
              <li className="flex items-center gap-3 text-brand-black font-medium">
                <Check className="w-5 h-5 text-brand-accent" /> Minimum 1 night stay
              </li>
              <li className="flex items-center gap-3 text-brand-black font-medium">
                <Check className="w-5 h-5 text-brand-accent" /> Available year-round
              </li>
              <li className="flex items-center gap-3 text-brand-black font-medium">
                <Check className="w-5 h-5 text-brand-accent" /> Per night pricing
              </li>
              <li className="flex items-center gap-3 text-brand-black font-medium">
                <Check className="w-5 h-5 text-brand-accent" /> No deposit required for enquiry
              </li>
            </ul>
          </motion.div>
          <motion.div variants={slideRight} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <form className="bg-brand-surface rounded-2xl p-8 space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <textarea placeholder="Message or Special Requests" className="w-full bg-white border border-brand-rule rounded-xl px-5 py-3.5 text-sm text-brand-black placeholder:text-brand-muted focus:outline-none focus:border-brand-accent focus:ring-2 focus:ring-brand-accent/15 transition-all duration-200 resize-none h-32"></textarea>
      ...
      <section className="bg-brand-surface py-20 px-5">
        <div className="max-w-3xl mx-auto">
          <h3 className="text-2xl font-bold text-brand-black mb-8 text-center">House Rules</h3>
          <ul className="space-y-4 bg-white rounded-2xl p-8 border border-brand-rule">
            ...
            <li className="flex justify-between items-center border-b border-brand-rule pb-4">
              <span className="font-medium text-brand-black">Maximum occupancy</span>
            ...
            <li className="flex justify-between items-center border-b border-brand-rule pb-4">
              <span className="font-medium text-brand-black">Check-in / Check-out</span>
              <span className="text-brand-mid">2:00 PM / 11:00 AM</span>
            </li>
            <li className="flex justify-between items-center pt-2">
              <span className="font-medium text-brand-black">Cancellation policy</span>
              <span className="text-brand-mid">Contact directly for details</span>
            </li>
          </ul>
        </div>
      </section>
