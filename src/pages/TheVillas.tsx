...
import { fadeUp, slideLeft, slideRight, staggerContainerSlow, staggerContainer, scaleUp } from '../lib/animations'
import { villaData, sharedAmenities } from '../data/villas'
import { Waves, Fish, TreePine, Utensils, Flame, Users, Sparkles, ShieldCheck } from 'lucide-react'
  ...
  return (
    <PageTransition>
      ...
      >
        ...
        >
          <motion.p variants={fadeUp} className="text-xs font-semibold uppercase tracking-widest text-white/60 mb-6">
            Familia Furtado
          </motion.p>
          <AnimatedHeading 
            text="The Villas" 
          ...
          <motion.p variants={fadeUp} className="text-lg text-white/80 max-w-lg mx-auto leading-relaxed">
            Two private retreats. Same soul, same address. Both available independently or together.
          </motion.p>
        </motion.div>
      </ParallaxSection>
            ...
            >
              <div className="rounded-2xl overflow-hidden aspect-[4/3] mb-8">
                <motion.img 
                  ...
                  src={index === 0 ? "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?ixlib=rb-4.0.3&auto=format&fit=crop&w=1600&q=80" : "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?ixlib=rb-4.0.3&auto=format&fit=crop&w=1600&q=80"} 
                  alt={villa.name}
                  className="w-full h-full object-cover"
                />
              </div>
              <h2 className="text-3xl font-bold text-brand-black mb-4">{villa.name}</h2>
              <p className="text-brand-mid mb-6">
                {villa.bedrooms} Bedrooms · {villa.bathrooms} Bathrooms · Up to {villa.maxOccupancy} Guests · {villa.balconies} Balconies
              </p>
              ...
              <div>
                <Button to={`/villas/${villa.id}`} variant="primary">Explore This Villa &rarr;</Button>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <motion.div variants={slideRight} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <div className="flex items-center gap-4 mb-4">
              ...
              />
              <span className="text-xs font-semibold uppercase tracking-widest text-brand-muted">The Full Estate</span>
            </div>
            <h2 className="text-4xl xl:text-5xl font-bold leading-tight mb-6">One Compound. Twelve Guests. Zero Compromises.</h2>
            <p className="text-base font-normal leading-relaxed text-brand-mid mb-6">
              When the whole family comes to Goa — or when a wedding calls for something extraordinary — both villas open as a single private estate. Shared pool, shared grounds, two fully staffed homes.
            </p>
            <p className="text-lg font-medium text-brand-black mb-8">
              12 Guests · 4 Bedrooms · 6 Bathrooms · 14m Pool · 8 Balconies
            </p>
            <Button to="/contact" variant="primary">Enquire About the Full Estate &rarr;</Button>
          </motion.div>
        </div>
      </section>
          ...
          >
            <div className="flex items-center gap-4 mb-4">
              <motion.div
                ...
                className="h-px w-12 bg-brand-rule origin-left"
              />
              <span className="text-xs font-semibold uppercase tracking-widest text-brand-muted">Shared Between Both Villas</span>
              <motion.div
                initial={{ scaleX: 0 }}
                whileInView={{ scaleX: 1 }}
                viewport={{ once: true }}
            ...
            <h2 className="text-4xl xl:text-5xl font-bold leading-tight mb-4">The Grounds Are Yours</h2>
          </motion.div>
      ...
      <section className="bg-brand-black py-24 px-5">
        ...
        >
          <h2 className="text-4xl xl:text-5xl font-bold text-white mb-6">Ready to make it yours?</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button to="/contact" variant="primary">Start the Conversation &rarr;</Button>
            <Button to="#" variant="outlineWhite" className="border-white/20 text-white/70 hover:bg-white/10 hover:text-white hover:border-white/30">View on Airbnb</Button>
          </div>
        </motion.div>
      </section>
    </PageTransition>
